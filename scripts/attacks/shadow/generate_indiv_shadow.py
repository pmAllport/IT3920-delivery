...
shadow = ShadowAttack(estimator=classifier, batch_size=1,nb_steps=600, learning_rate=0.05)
...
def main():
    ...
    for x, y in tqdm(loader):
        ...
        for img,gt in zip(x,y):
            np_image = img.unsqueeze(0).clone().numpy()
            
            shadow_img = shadow.generate(x=np_image).squeeze(0)

            shadow_img = shadow_img.transpose(1,2,0)
            shadow_img = np.clip(shadow_img,0,1,dtype=np.float32)
            
            
            image_bgr = cv2.cvtColor(shadow_img, cv2.COLOR_RGB2BGR,cv2.CV_32FC3)
            cv2.imwrite(
                img=image_bgr,
                filename=f"{args.targetlocation}/{counter}_shadow.tiff",
