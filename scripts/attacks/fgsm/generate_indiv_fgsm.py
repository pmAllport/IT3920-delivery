...
fastGradient = FastGradientMethod(estimator=classifier, eps=0.05, batch_size=dataset_batch_size,targeted=False,eps_step=0.0001, norm=2)
...
def main():
    ...
    for x, y in tqdm(loader):
        fgsm_imgs = fastGradient.generate(
            x=x.clone().numpy()
        )


        for _,fgsm in zip(y,fgsm_imgs):
            
            fgsm = fgsm.transpose(1,2,0)
            fgsm = np.clip(fgsm,0,1,dtype=np.float32)
            
            
            image_bgr = cv2.cvtColor(fgsm, cv2.COLOR_RGB2BGR,cv2.CV_32FC3)
            cv2.imwrite(
                img=image_bgr,
                filename=f"{args.targetlocation}/{counter}_fgsm.tiff",
