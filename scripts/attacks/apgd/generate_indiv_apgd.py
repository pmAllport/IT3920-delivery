...
auto_projected = AutoProjectedGradientDescent(estimator=classifier,eps=0.05, batch_size=dataset_batch_size,eps_step=0.0001,targeted=False, norm=2)
...
def main():
    ...
    for x, y in tqdm(loader):
        apgd_imgs = auto_projected.generate(
            x=x.clone().numpy(),
            y=y.clone().numpy(),
        )


        for _,apgd in zip(y,apgd_imgs):
            
            apgd = apgd.transpose(1,2,0)
            apgd = np.clip(apgd,0,1,dtype=np.float32)
            
            
            image_bgr = cv2.cvtColor(apgd, cv2.COLOR_RGB2BGR,cv2.CV_32FC3)
            cv2.IMWRITE_TIFF_COMPRESSION = 0
            cv2.imwrite(
                img=image_bgr,
                filename=f"{args.targetlocation}/{counter}_apgd.tiff",
                params=[cv2.CV_32FC3,cv2.IMWRITE_TIFF_COMPRESSION],
            )
            counter +=1
